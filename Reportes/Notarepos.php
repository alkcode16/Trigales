...
if($venta->id<10){
	$ceros="000";
}else if($venta->id<100){
	$ceros="00";
}else if($venta->id<1000){
	$ceros="0";
}else{
	$ceros="";
}
...
//////////////////////////////////////////////////CABECERA DE LA NOTA///////////////////////////////////////////////////
  $pdf->SetXY(182,14);
  $pdf->Cell(70,6,utf8_decode($ceros."".$venta->id),0,0,'L');
...
$pdf->SetXY(10,56);
foreach (explode("__",utf8_decode($venta->pasteles)) as $productosConcatenados) {
    $producto=explode("..", $productosConcatenados);

      // code...
      $pdf->Cell(30,6,utf8_decode($producto[0]),0,"Cell",'C');
      $pdf->Cell(80,6,utf8_decode($producto[1]),0,"Cell",'L');
...
/*foreach($_SESSION["carrito"] as $indice => $producto){
      ...
      //precio parcial
      $caracter="$producto->p_temporada";
      if (substr($producto->p_temporada, -3)==".00") {

      }else {

          if (substr($caracter, -2)==".5"){
            $producto->p_temporada=$producto->p_temporada."0";
          }else {
            $producto->p_temporada=$producto->p_temporada.".00";
          }
      }
      $pdf->Cell(40,6,utf8_decode("$".$producto->p_temporada),0,"Cell",'R');
      //totales
...
$pdf->Output("NOTA ".$ceros."".$venta->id.".pdf","D");
